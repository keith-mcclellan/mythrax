...
async fn get_node_scope(backend: &SurrealBackend, id: &str) -> String {
    if let Ok(rec_id) = parse_record_id(id) {
        let sql = "SELECT VALUE scope FROM type::table($tb) WHERE id = $id LIMIT 1;";
        if let Ok(mut response) = backend.db.query(sql).bind(("tb", rec_id.table.to_string())).bind(("id", rec_id)).await {
