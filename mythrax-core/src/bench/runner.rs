...
use mythrax_core::bench::metrics::{
    evaluate_retrieval, parse_haystack_date, session_id_from_corpus_id,
};
...
async fn main() -> Result<()> {
    ...
    if let Err(e) = mythrax_core::embeddings::load_embedding_cache_from_disk(&target_cache_path) {
        ...
        if args.mode == "tune" {
            ...
            println!(
                "Recall_All@25 Ceiling:    {:.4}",
                avg_ceiling_recall_all_turn_at25
            );
            println!("nDCG@{}:                  {:.4}", K_NDCG, avg_ndcg_turn);
            println!(
                "nDCG@{} Ceiling:          {:.4}",
                K_NDCG, avg_ceiling_ndcg_turn_at10
            );
            ...
            println!(
                "Recall_Any@{} Session Ceiling: {:.4}",
                K_RECALL, avg_ceiling_recall_any_session_at5
            );
            println!(
                ...
                let baseline_content = format!(
                    ...
                    avg_recall_all_turn,
                    {
                        let avg_recall_all_turn_at25 = if records.is_empty() {
                            ...
                            records.iter().map(|r| r.recall_all_turn_at25).sum::<f32>()
                                / records.len() as f32
                        };
...
async fn run_evaluation(
    ...
    if let Ok(embedder) = mythrax_core::embeddings::LocalEmbedder::get_global() {
        let _ = embedder.embed("warmup query for metal vram").await;
        let _ = embedder
            .embed_batch(&["warmup batch query for metal vram".to_string()])
            .await;
    }
