...
fn boosts_clamp_to_zero_two_range() {
    ...
    let d = apply_boosts(0.10, &BoostSignals { person_name: true, exact_quote: true, temporal_proximity: 0.0, keyword_overlap: 0.0 }, &w);
    assert!(d >= 0.0 && d <= 2.0);
}
...
fn person_name_reduces_distance_about_40pct() {
    let w = BoostWeights::default();
    let base = 1.0;
    let boosted = apply_boosts(base, &BoostSignals { person_name: true, exact_quote: false, temporal_proximity: 0.0, keyword_overlap: 0.0 }, &w);
    assert!(boosted < base);
...
fn quoted_phrase_reduces_distance_about_60pct() {
    let w = BoostWeights::default();
    let base = 1.0;
    let boosted = apply_boosts(base, &BoostSignals { person_name: false, exact_quote: true, temporal_proximity: 0.0, keyword_overlap: 0.0 }, &w);
    assert!(boosted < base);
