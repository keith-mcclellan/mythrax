...
*   **Strict Anti-Lazy Implementation Mandate:** Coding subagents and developers are strictly forbidden from taking lazy design shortcuts, writing temporary sync/async bridge wrappers, inserting `// TODO` or placeholder stubs, omitting error handling on edge cases, or applying band-aid patches to pass individual tests. All implementations must be fully written out, architecturally sound, type-safe, and natively integrated across all downstream callsites.
...
*   **Incremental Per-Phase Git Commit & Push Mandate:** Subagents and developers MUST execute a git commit and `git push origin <branch_name>` immediately upon completing each phase of a track (after verifying unit tests and build status) before proceeding to subsequent phases or triggering code reviews. This prevents multi-commit push backlogs and keeps remotes continuously up to date.
