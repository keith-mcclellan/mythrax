                ...
                if now_unix - last_hb > 60 {
                    distiller_warning = format!(
                        "\n> [!IMPORTANT]\n\
                         ...
                         > `invoke_subagent(TypeName=\"self\", Role=\"Mythrax Cloud Brain\", Prompt=\"You are the Mythrax Cloud Brain. Your job is to orchestrate the bootstrap, dreaming, direction ideation, wisdom graduation, vault organization, and cognitive callback resolution for Mythrax using MCP endpoints. IMPORTANT RULES: 1. CRITICAL SYNTAX: In your call_mcp_tool calls, the ServerName argument MUST be the exact, unquoted, bare string mythrax, and the ToolName argument MUST be the exact, unquoted, bare string manage or write. Do NOT use escaped quotes like \\\"\\\\\\\"mythrax\\\\\\\"\\\" or add any quotes inside the JSON string arguments. This is a strict syntax requirement. If you fail to do this, the system will deadlock. 2. You MUST ONLY use call_mcp_tool endpoints to query, summarize, and resolve callbacks. 3. You MUST NEVER run any daemon start, stop, kill, or other command-line tools under any circumstances. 4. If the daemon is unreachable (returns EOF or connection closed), simply wait and try again. 5. Run in an infinite loop. Even if the pre_invocation check returns 0 pending callbacks, DO NOT stop or exit the loop. Sleep/wait for 5 seconds and query again. You must run continuously in this check-sleep cycle forever until explicitly terminated by the parent. Please execute the following sequence: 1. PHASE 1: Bootstrap & Embedding Catch-Up via MCP: Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{action: \\\"bootstrap\\\", force: true, async_mode: true}}. Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{action: \\\"reprocess\\\"}}. 2. PHASE 2: Dynamic Scope Dreaming & Direction Ideation Trigger via MCP: Call call_mcp_tool with ServerName='mythrax', ToolName='read', Arguments={{action: \\\"scopes\\\"}} to retrieve all active scopes across SurrealDB and the Obsidian vault. To prevent macOS Metal GPU timeout/hang crashes, trigger summarization and direction ideation for EACH discovered scope SEQUENTIALLY (waiting for each MCP tool call to return before starting the next) with async_mode=false: For each scope in discovered_scopes: Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{action: \\\"summarize\\\", scope: '<scope_name>', async_mode: false}}. Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{action: \\\"ideate\\\", scope: '<scope_name>', hypothesis: 'Auto-synthesize research directions for scope'}}. 3. PHASE 3: Wisdom Graduation & Vault Organization via MCP: Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{action: \\\"audit_compliance\\\"}}. Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{action: \\\"organize\\\"}}. 4. PHASE 4: Continuous Callback Resolution Loop via MCP: Run in an infinite loop: Call call_mcp_tool with ServerName='mythrax', ToolName='manage', Arguments={{session_id: \\\"{}\\\", action: \\\"pre_invocation\\\", caller: \\\"distiller\\\"}}. Read the output. Look for the section '### 🧠 Pending Cognitive Callbacks'. For each task in that section: Extract the Callback ID, system instruction, and prompt. Using your cloud brain, generate the output. Call call_mcp_tool write cognitive_callback... Sleep/wait for 5 seconds and repeat.\")`\n",
                        session_id
                    );
                }
            }
