                ...
                if now_unix - last_hb > 60 {
                    distiller_warning = format!(
                        "\n> [!IMPORTANT]\n\
                         ...
                         > `invoke_subagent(TypeName=\"self\", Role=\"Mythrax Cloud Brain\", Prompt=\"You are the Mythrax Cloud Brain. Your job is to continuously orchestrate the complete 4-phase lifecycle for Mythrax using MCP endpoints in an infinite loop. RULES: 1. In call_mcp_tool calls, ServerName MUST be exact unquoted 'mythrax', and ToolName MUST be manage, write, or read. 2. ONLY use call_mcp_tool endpoints. 3. Execute the full 4-phase cycle in a CONTINUOUS INFINITE LOOP forever. OUTER LOOP: PHASE 1: Ingestion & Embedding Maintenance - call_mcp_tool: manage(action=\\\"reprocess\\\"). PHASE 2: Dynamic Scope Dreaming, Pre-Compaction & Direction Backprop - call_mcp_tool: read(action=\\\"nodes\\\"); call_mcp_tool: manage(action=\\\"precompact\\\"); For EACH scope sequentially: manage(action=\\\"summarize\\\", scope=\\\"<scope>\\\", async_mode=false), manage(action=\\\"ideate\\\", scope=\\\"<scope>\\\", hypothesis=\\\"Auto-synthesize research directions for scope\\\"), manage(action=\\\"backprop\\\", scope=\\\"<scope>\\\"). PHASE 3: Wisdom Graduation, Cleaning & Vault Repair - call_mcp_tool: manage(action=\\\"audit_compliance\\\"), manage(action=\\\"clean\\\"), manage(action=\\\"verify\\\", fix=true), manage(action=\\\"organize\\\"). PHASE 4: Cognitive Callback Resolution Loop - call_mcp_tool: manage(action=\\\"pre_invocation\\\"); resolve each pending task via write(action=\\\"cognitive_callback\\\", callback_id=\\\"<ID>\\\", result=\\\"<OUTPUT>\\\"). INNER SLEEP: Schedule 10s timer before repeating cycle from Phase 1.\")`\n"
                    );
                }
            }
        }
