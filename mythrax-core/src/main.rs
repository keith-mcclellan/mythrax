...
use mythrax_core::{cli, daemon, db, mcp};
        ...
        Commands::Init {
            ...
            let (old_scope_mappings, old_skip_scopes) = if config_path.exists() {
                if let Ok(content) = std::fs::read_to_string(&config_path) {
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&content) {
                        (
                            val.get("scope_mappings").cloned(),
                            val.get("skip_scopes").cloned(),
                        )
                    } else {
                        (None, None)
                    }
            ...
            if (!onnx_path.exists() && !mlx_path.exists()) || !tokenizer_path.exists() {
                println!(
                    "WARNING: Nomic embedding model files (model.safetensors or nomic-embed-text-v1.5.onnx) not found under ~/.mythrax/models/. Local embeddings will fallback to None."
                );
            }
        ...
        Commands::Vault { action } => {
            let (act_str, args) = match action {
                ...
                VaultAction::Summarize { scope } => (
                    "summarize",
                    serde_json::json!({ "scope": scope, "async_mode": false }),
                ),
                VaultAction::IngestBulk {
                    ...
                    "ingest_bulk",
                    serde_json::json!({ "source": source, "harness": harness, "scope": scope, "async_mode": false }),
                ),
                VaultAction::IngestForge { source_path, scope } => (
                    "ingest_forge",
                    serde_json::json!({ "source_path": source_path, "scope": scope, "async_mode": false }),
                ),
                ...
                VaultAction::Clean { dry_run, confirm } => {
                    if dry_run {
                        (
                            "clean",
                            serde_json::json!({ "dry_run": true, "confirm": false }),
                        )
                    } else if confirm {
                        (
                            "clean",
                            serde_json::json!({ "dry_run": false, "confirm": true }),
                        )
                    } else {
                        let home = std::env::var("HOME").context("HOME env var not set")?;
                        let token_path = std::path::PathBuf::from(&home)
                            ...
                            .join("token");
                        let auth_token = mythrax_core::auth::get_or_create_token(&token_path)?;
                        let daemon_port = std::env::var("MYTHRAX_DAEMON_PORT")
...
async fn config_harness_action(harness: &str, _vault_root: &std::path::Path) -> Result<()> {
    let home = std::env::var("HOME").context("HOME env var not set")?;
