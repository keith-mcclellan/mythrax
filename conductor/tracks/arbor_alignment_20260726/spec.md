...
### M. Nuclear Memory Wipe Default (`manage_handlers.rs` L1810-1841)
- **Current**: `MYTHRAX_PRE_INVOCATION_TOKEN_BUDGET` defaults to 3000 tokens. Playbook + preamble + policies consume 1000+, leaving <2000 for memories. When exceeded, `p1_advisory.clear()` wipes ALL memories.
- **Required**: Change hardcoded default from `3000` to `32000`. Replace `.clear()` with graduated truncation: truncate longest memories first → summarize → drop lowest-relevance one at a time → NEVER drop ALL. Keep env var as override, but default must be safe out of the box.
...
### Q. Missing Embedding Reprocessor (`crud_operations.rs` L456)
- **Current**: `MYTHRAX_ASYNC_EMBEDDINGS` env var skips inline embedding generation. There is NO background catch-up worker. `grep reprocess_missing_embeddings` = 0 hits. If this env var is set, embeddings are permanently lost.
- **Required**: Implement background reconciliation sweep: query all records where `embedding IS NONE`, generate embeddings, update. This also closes the RAPTOR embedding gap.

### R. Cognitive Task Queue Polling Bottleneck (`distillation.rs` L187-198)
- **Current**: `run_summarization_task` spawns a tight 50ms polling loop per task. During bulk summarization, hundreds of concurrent loops hammer SurrealDB.
- **Required**: Replace with channel/notification mechanism or `tokio::sync::watch` for task completion.

### S. Search Content Slicing Bug
- **Current**: Search result summary snippets are generated by splitting content with `.split(&['.', '!', '?'])`. Since content is markdown/JSON, this slices mid-URL or mid-code-block, returning garbage fragments.
- **Required**: Use sentence-aware or word-boundary-aware truncation. Fold into search result formatting fix.

### T. Prioritized Fix Order (Maximum Impact, Minimum Code)
1. **Code fix**: Change token budget default from 3000→32000 AND replace `.clear()` with graduated truncation (`manage_handlers.rs`)
2. **Small**: Format search results as markdown + fix content slicing (`read_handlers.rs`)
3. **Small**: Raise STM truncation from 1000 to 32000 chars (`manage_handlers.rs` L98)
...
6. **Medium**: Implement embedding reprocessor for `embedding IS NONE` records
7. **Medium**: Replace cognitive task polling loops with notifications
