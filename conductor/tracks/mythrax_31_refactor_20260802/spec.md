...
### 2.4 MCP Tool Schema & Pre-Flight Memory Gate Enforcement
- **Programmatic Pre-Flight Memory Gate:** Mythrax MCP Tool Router (`mcp_routes.rs`) will track session memory check status (`has_checked_memory`). Any attempt by an agent or subagent to call write/modifying tools (`write_file`, `replace_file_content`, `run_command`, `write`, `organize`) before executing `read(action="search")` or `manage(action="pre_invocation")` will be intercepted with a high-priority enforcement directive.
- **Automatic Subagent Handoff Wrapper:** All subagent prompts generated by system handlers will automatically prepend a mandatory `PHASE 0 MEMORY SEARCH` instruction.
