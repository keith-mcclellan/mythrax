...
mod tests {
    ...
    async fn test_watcher_sync() {
        ...
        let results = backend.search("Body content", Some("watcher-testing"), false, 1, 0, 0.55, None, false).await.unwrap();
    ...
    async fn test_bidirectional_sync_and_loop_prevention() {
        ...
        let results = backend.search("bidirectional sync", Some("bi-testing"), false, 1, 0, 0.55, None, false).await.unwrap();
        assert_eq!(results.results.len(), 1);
        ...
        let results2 = backend.search("updated body", Some("bi-testing"), false, 1, 0, 0.55, None, false).await.unwrap();
