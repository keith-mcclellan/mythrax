...
impl TargetResolveCache {
    pub async fn new(backend: &crate::db::SurrealBackend) -> Self {
        ...
        // 1. Fetch wiki_nodes
        if let Ok(mut resp) = backend
            .db
            .query("SELECT id, name, vault_path FROM wiki_node;")
            .await
        {
        ...
        if let Ok(mut resp) = backend
            .db
            .query("SELECT id, title, vault_path FROM episode;")
            .await
        {
            #[derive(serde::Deserialize, surrealdb_types::SurrealValue)]
            struct EpisodeLight {
        ...
        if let Ok(mut resp) = backend
            .db
            .query("SELECT id, target_pattern, vault_path FROM wisdom;")
            .await
        {
            #[derive(serde::Deserialize, surrealdb_types::SurrealValue)]
            struct WisdomLight {
...
pub async fn sync_file_to_db_with_cache(
    ...
    if rel_path.contains("episodes/") {
        ...
        if let Some(yaml_val) = yaml_opt {
            ...
            let final_tier_enum = final_tier
                ...
                backend.as_any().downcast_ref::<crate::db::SurrealBackend>()
            {
                if let Ok(from_id) = crate::db::parse_record_id(&db_id) {
                    let body_links = crate::parser::extract_wiki_links(&body);
                    let mut desired: Vec<(surrealdb::types::RecordId, String, Option<f32>)> =
                        Vec::new();
        ...
        if let Some(surreal_backend) = backend.as_any().downcast_ref::<crate::db::SurrealBackend>()
        {
            ...
            for ext in &existing {
                let is_still_desired = desired
                    .iter()
                    .any(|(tid, rel, _)| tid == &ext.out && Some(rel) == ext.relation.as_ref());
                if !is_still_desired {
                    let delete_q = "DELETE FROM relates_to WHERE id = $rel_id;";
                    let _ = surreal_backend
                        .db
            ...
            for (tid, rel, strength_opt) in desired {
                let already_exists = existing
                    .iter()
                    .any(|ext| &ext.out == &tid && ext.relation.as_ref() == Some(&rel));
                if !already_exists {
                    let relate_q = "RELATE $from->relates_to->$to CONTENT { relation: $relation, strength: $strength };";
                    let _ = surreal_backend
                        .db
...
mod tests {
    ...
    async fn test_watcher_sync_skills_tier() {
        ...
        let results = backend
            .get_wisdom(
                "test-pattern",
                Some(crate::contracts::Tier::Wisdom),
                10,
                0,
                0.0,
            )
