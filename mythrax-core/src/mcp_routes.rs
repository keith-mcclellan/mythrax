...
async fn handle_manage(state: &ApiState, args: Value) -> Result<Value> {
    ...
    match mapped_action {
        ...
        | "ingest_forge" | "save_forged_assets" => {
            match mapped_action {
                "ingest_bulk" => {
                    let _source = args
                        .get("source")
        ...
        "timeline" => {
            ...
            let resolved_anchor_id = if let Some(id) = anchor_id {
                ...
                let search_res = state
                    .backend
                    .search(
                        q, None, false, 1, 0, 0.0, None, false, true, false, None, true,
                    )
                    .await?;
                let best = search_res
                    .results
            ...
            if has_correction {
                    ...
                    state.backend.as_any().downcast_ref::<SurrealBackend>()
                {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) =
                            run_llm_critic(backend_clone, store_clone, content_clone, scope_clone)
                                .await
                        {
    ...
    match action {
        ...
        "summarize" => {
            ...
            compactor
                ...
                .await?;
            compactor
                .compact_global(&*state.backend, &state.store)
                .await?;
...
pub async fn handle_pre_invocation_hook(state: &ApiState, args: Value) -> Result<Value> {
    ...
    let capabilities_res = surreal_backend
        .db
    ...
    if !handoffs.is_empty() {
        ...
        if !node_ids.is_empty() {
            ...
            for ep in hydrated.episodes {
                ...
                if let Some(ref ep_id) = ep.id {
                    let rendered = format_episode_or_parent(
                        &*state.backend,
                        &surreal_backend.db,
                        ep_id,
                        &ep.title,
            ...
            let search_res = state
                .backend
                .search(
                    summary, scope, false, 15, 0, 0.55, None, false, true, false, None, true,
                )
                .await?;
        ...
        parts.push(format!(
            "## Retrieved Semantic Context (Scope: `{}`)\n",
            dynamic_scope
        ));
    ...
    if let Some(active_node_id) = active_node_opt {
        let mut hyp_res = surreal_backend
            .db
        ...
        if let Some(hyp_node) = hyp_nodes.first() {
            if let Some(ref parent_id) = hyp_node.parent_id {
                ...
                if !siblings.is_empty() {
                    ...
                    for sib in siblings {
                        let status_label = if sib.status == "failed" {
                            "FAILED"
                        } else {
                            "PRUNED"
                        };
                        let reason = sib
                            .result
                            .or(sib.insight)
                            .unwrap_or_else(|| "No failure details recorded".to_string());
                        constraint_parts.push(format!(
                            "- **Approach to Avoid**: `{}` (Status: {})\n  - **Reason**: {}",
                            sib.hypothesis, status_label, reason
                        ));
                    }
