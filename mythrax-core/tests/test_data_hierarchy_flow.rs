...
async fn test_data_hierarchy_flow_ingest_and_retrieve() {
    ...
    let search_res = backend.search("execution context", Some("general"), false, 5, 0, 0.1, None, false, true, false).await.unwrap();
    ...
    let search_raptor = backend.search("Raptor Summary", Some("general"), false, 5, 0, 0.1, None, false, false, false).await.unwrap();
    ...
    let search_insight = backend.search("permanent wiki nodes", Some("general"), false, 5, 0, 0.1, None, false, false, false).await.unwrap();
