        ...
        match req.send().await {
            Ok(resp) => {
                ...
                (
                    status,
                    header_map,
                    axum::body::Body::from_stream(intercepted_stream),
                )
                    .into_response()
            }
            Err(e) => (
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                match resp.json::<Value>().await {
                    Ok(mut json_val) => {
                        if let Some(choices) =
                            json_val.get_mut("choices").and_then(|c| c.as_array_mut())
                        {
        ...
        loop {
            match Pin::new(&mut self.inner).poll_next(cx) {
                Poll::Ready(Some(Ok(bytes))) => {
                    ...
                    let mut pos = 0;
                    while let Some(newline_idx) =
                        self.buffer[pos..].iter().position(|&b| b == b'\n')
                    {
