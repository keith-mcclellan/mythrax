...
fn test_inspect_calibrated_score_pairs() {
    ...
    let irrelevant_passage = "The President of the United States is the head of state and head of government.";
    ...
    println!("Original max_seq_len: {}, Bucketed to: {}", max_seq_len, max_seq_len_bucketed);
    ...
    let causal_mask = mlx_rs::ops::full::<f32>(&[max_seq_len_i32, max_seq_len_i32], &Array::from(f32::NEG_INFINITY)).unwrap();
    ...
    let mask = mask.reshape(&[batch_size as i32, 1, max_seq_len_i32, max_seq_len_i32]).unwrap();
    ...
    let logit_no = last_hidden.multiply(&w_no_tok).unwrap().sum_axes_device(&[-1], false, StreamOrDevice::gpu()).unwrap();
    let logit_yes = last_hidden.multiply(&w_yes_tok).unwrap().sum_axes_device(&[-1], false, StreamOrDevice::gpu()).unwrap();
    ...
    let exp_no = calibrated_logit_no.subtract(&max_logit).unwrap().exp().unwrap();
    let exp_yes = calibrated_logit_yes.subtract(&max_logit).unwrap().exp().unwrap();
    ...
    assert!(scores[0] > scores[1], "Relevant passage must score higher than irrelevant passage");
}
