...
fn person_name_reduces_distance_about_40pct() {
    let w = BoostWeights::default();
    let base = 1.0;
    let boosted = apply_boosts(base, &BoostSignals { person_name: true, exact_quote: false, temporal_proximity: 0.0, keyword_overlap: 0.0, ..Default::default() }, &w);
    assert!(boosted < base);
...
fn quoted_phrase_reduces_distance_about_60pct() {
    let w = BoostWeights::default();
    let base = 1.0;
    let boosted = apply_boosts(base, &BoostSignals { person_name: false, exact_quote: true, temporal_proximity: 0.0, keyword_overlap: 0.0, ..Default::default() }, &w);
    assert!(boosted < base);
