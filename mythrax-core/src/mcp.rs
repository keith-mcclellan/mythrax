    ...
    async fn ensure_daemon_active(&self) -> Result<()> {
        ...
        match cmd.spawn() {
            Ok(child) => {
                ...
                eprintln!(
                    "Daemon successfully spawned (PID: {}) and bound to port 8090.",
                    pid
                );
                Ok(())
            }
...
```";
        ...
        if let Some(ref local) = self.local_state {
            ...
            if resp.status() != reqwest::StatusCode::OK {
                anyhow::bail!(
                    "Daemon returned error status calling tool '{}': {}",
                    name,
                    resp.status()
                );
            }
        ...
        if let Some(ref _local) = self.local_state {
            match method {
                ...
                "tools/call_batch" => {
                    let calls_val = if params.is_array() {
                        ...
                        params
                            .get("calls")
                            .ok_or_else(|| anyhow::anyhow!("Missing 'calls' parameter"))?
                    };
                    let calls_arr = calls_val
                        .as_array()
                ...
                "resources/read" => {
                    let uri = params
                        .get("uri")
                        .and_then(|v| v.as_str())
                        .context("Missing uri in resources/read")?;
                    if uri == "htr://tree" {
            ...
            match method {
                ...
                "tools/list" => {
                    ...
                    if resp.status() != reqwest::StatusCode::OK {
                        anyhow::bail!(
                            "Daemon returned error status listing tools: {}",
                            resp.status()
                        );
                    }

                    let tools_list: Value = resp
                        .json()
                        .await
                        .context("Failed to parse daemon tools JSON")?;
                    Ok(tools_list)
                }
                "tools/call" => {
                    let name = params
                    ...
                    let call_result: Value = resp
                        .json()
                        .await
                        .context("Failed to parse daemon tool call result JSON")?;
                    Ok(call_result)
                }
                "tools/call_batch" => {
                    let calls_val = if params.is_array() {
                ...
                "resources/list" => {
                    ...
                    if resp.status() != reqwest::StatusCode::OK {
                        anyhow::bail!(
                            "Daemon returned error status listing resources: {}",
                            resp.status()
                        );
                    }
                    let resources_list: Value = resp
                        .json()
                        .await
                        .context("Failed to parse daemon resources JSON")?;
                    Ok(resources_list)
                }
                "resources/read" => {
                    let uri = params
                    ...
                    if resp.status() != reqwest::StatusCode::OK {
                        anyhow::bail!(
                            "Daemon returned error status reading resource '{}': {}",
                            uri,
                            resp.status()
                        );
                    }
                    let read_result: Value = resp
                        .json()
                        .await
                        .context("Failed to parse daemon resource read result JSON")?;
                    Ok(read_result)
                }
                _ => anyhow::bail!("Method not found: {}", method),
            }
