...
| `manage_vault` | **Action**: `verify` \| `organize` \| `reprocess` \| `summarize` \| `audit`<br>**Params**: `fix?`, `scope?`, `workspace_path?` | Run DB-to-filesystem self-healing (`verify`), organize/deduplicate vault files (`organize`), reprocess missing embeddings (`reprocess`), summarize/dream over memories (`summarize`), or run compliance audits (`audit`). |
