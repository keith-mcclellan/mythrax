...
fn split_by_chars(text: &str, max_chars: usize) -> Vec<String> {
    ...
    for c in text.chars() {
        if count >= max_chars {
            // PERF: Replace String instead of clone + clear to prevent unnecessary allocations
...
fn group_sub_chunks(sub_chunks: Vec<String>, delimiter: &str, max_chars: usize) -> Vec<String> {
    ...
    for chunk in sub_chunks {
        ...
        if needed_len <= max_chars {
            ...
            if !current_group.is_empty() {
                // PERF: Replace String instead of clone and manual variable reassignment to prevent allocation
                grouped.push(std::mem::replace(&mut current_group, String::with_capacity(max_chars)));
            }
            current_group.push_str(&chunk);
