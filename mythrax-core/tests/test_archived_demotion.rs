...
async fn test_archived_demotion_logic() -> Result<()> {
    ...
    let mut all_eps_res = backend.db.query("SELECT id, title, session_id, archived, archived_at FROM episode;").await?;
    ...
    println!("CROSS-SESSION RESULTS: {:?}", results_cross.iter().map(|r| &r.title).collect::<Vec<_>>());
    assert_eq!(results_cross.len(), 7, "Should return all 7 search results for cross-session search");

    let get_score_cross = |id: &str| -> f32 {
        results_cross.iter().find(|r| r.id == id).map(|r| r.similarity).unwrap_or(0.0)
    };
    ...
    println!("CROSS-SESSION Recent Score (expect demoted, ratio ~0.4): {}", score_recent_cross);
    println!("CROSS-SESSION One Day Score (expect demoted, ratio ~0.4): {}", score_one_day_cross);
    ...
    let get_score_same = |id: &str| -> f32 {
        results_same.iter().find(|r| r.id == id).map(|r| r.similarity).unwrap_or(0.0)
    };
    ...
    println!("SAME-SESSION Recent Score (expect same-session bypass, ratio ~1.0): {}", score_recent_same);
    ...
    assert!((ratio_recent_same - 1.0).abs() < 0.05, "Recent same-session ratio was {}", ratio_recent_same);
